...
data_C = load(fullfile(matPath_C, 'pxx_f-C-v.mat'));
data_L = load(fullfile(matPath_L, 'pxx_f-L-v.mat'));
...
for ipxx = 1:length(data_C.pxxs)
    ...
    epsname = sprintf("./figures/PSD-CL-v-%d.eps", ipxx);
    figname = sprintf("./figures/PSD-CL-v-%d.fig", ipxx);
    ...
    epsname = sprintf("./figures/pre-PSD-CL-v-%d.eps", ipxx);
    figname = sprintf("./figures/pre-PSD-CL-v-%d.fig", ipxx);
    saveas(pre_psd_CL, epsname, 'epsc');
    savefig(pre_psd_CL, figname);
    close(pre_psd_CL);
end

save("concate_pxx_f-v.mat", "concate_fs", "concate_pxxs", "concate_Ys");