...
Y_C = data_C.local_Ys(1:end-1); Y_L = data_L.local_Ys(4:end);
U_C = data_C.local_Us(1:end-1); U_L = data_L.local_Us(4:end);
urms_C = data_C.u_rms(1:end-1); urms_L = data_L.u_rms(4:end);
pxxs_C = data_C.pxxs(1:end-1); pxxs_L = data_L.pxxs(4:end);
fs_C = data_C.fs(1:end-1); fs_L = data_L.fs(4:end);
noise_f = 0.075:0.075:1; % 4.5:0转速比

concate_pxxs = cell(length(pxxs_C), 1);
concate_fs = cell(length(pxxs_C), 1);
concate_ks = cell(length(pxxs_C), 1);
