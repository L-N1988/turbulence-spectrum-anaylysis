...
for ipxx = 1:length(data.concate_pxxs)
    ...
    flim = 1; noise_f = 0.025:0.025:flim; % 1.5:0转速比
    ...
    set(ylabel("$S_{ww}(f) (\rm m^2/s)$"), 'Interpreter', 'latex');
    figtitle = sprintf("PSD at z=%.4f m", Y);
    set(title(figtitle), 'Interpreter', 'latex');
    legend([p1, p2, p3], {'raw', 'denoised', 'EMD'}, "FontSize", 12);
    % legend([p1, p2], {'raw', 'denoised'}, "FontSize", 12);
    
    epsname = sprintf("./concate_figures/PSD-concate-v-%d.eps", ipxx);
    figname = sprintf("./concate_figures/PSD-concate-v-%d.fig", ipxx);
    ...
    set(xlabel("$f$ (Hz)"), 'Interpreter', 'latex'); 
    set(ylabel("$fS_{ww}(f) (\rm m^2/s^2)$"), 'Interpreter', 'latex');
    figtitle = sprintf("pre-multiplied PSD at z=%.4f m", Y);
    set(title(figtitle), 'Interpreter', 'latex');
    legend([pre2, pre3], {'denoised', 'EMD'}, "FontSize", 12);
    % legend(pre2, {'denoised'}, "FontSize", 12);

    epsname = sprintf("./concate_figures/pre-PSD-concate-v-%d.eps", ipxx);
    figname = sprintf("./concate_figures/pre-PSD-concate-v-%d.fig", ipxx);
    saveas(pre_psd, epsname, 'epsc');
    savefig(pre_psd, figname);
    close(pre_psd);
end

save("denoise_concate_pxx_f-v.mat", "fs_denoise", "pxxs_denoise", "Ys", "emds");