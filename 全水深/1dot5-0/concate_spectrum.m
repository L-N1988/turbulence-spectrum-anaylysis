...
for ipxx = 1:length(data_C.pxxs)
    ...
    epsname = sprintf("./figures/PSD-CL-%d.eps", ipxx);
    figname = sprintf("./figures/PSD-CL-%d.fig", ipxx);
    ...
    epsname = sprintf("./figures/pre-PSD-CL-%d.eps", ipxx);
    figname = sprintf("./figures/pre-PSD-CL-%d.fig", ipxx);
    saveas(pre_psd_CL, epsname, 'epsc');
    savefig(pre_psd_CL, figname);
    close(pre_psd_CL);
end

save("concate_pxx_f.mat", "concate_fs", "concate_pxxs", "concate_Ys");