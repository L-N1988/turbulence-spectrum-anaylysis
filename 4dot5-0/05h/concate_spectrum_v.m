...
pre_C = plot(f_C(f_C <= 100), f_C(f_C <= 100) .* pxx_C(f_C <= 100), 'Color', blue);
hold on
pre_L = plot(f_L, f_L.* pxx_L, 'Color', orange);
pre_CL = plot(concate_f(concate_f <= 100), concate_f(concate_f <= 100) .* concate_pxx(concate_f <= 100), 'Color', green);
...
saveas(pre_psd_CL, 'pre-PSD-CL-v.eps', 'epsc');
savefig(pre_psd_CL, 'pre-PSD-CL-v.fig');

save("concate_pxx_f-v.mat", "concate_f", "concate_pxx", "urms_C", "U_C", "urms_L", "U_L");