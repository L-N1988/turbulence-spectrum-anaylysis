...
if contains(matPath, 'C')
    Fs = 150;
    window_len = [];
    spectrum_mat = 'pxx_f-C-v.mat';
elseif contains(matPath, 'L')
    Fs = 24;
    window_len = 16384;
    spectrum_mat = 'pxx_f-L-v.mat';
end
% Fs = 24;
v_fluc_tmp = zeros(lt, 1);
pxx = 0; f = 0;
v_fluc = 0; % fluctuation velocity at center
for i = -1:1:1
	for j = -1:1:1
		for k = 1:lt
			v_fluc_tmp(k) = v_pri{k}(center(1) + i, center(2) + j);
		end
		[pxx_, f_] = pwelch(v_fluc_tmp, window_len, [], [], Fs);
        pxx = pxx + pxx_; f = f + f_;
        v_fluc = v_fluc + v_fluc_tmp;
	end
end
pxx = pxx ./ 9; f = f ./ 9;
v_fluc = v_fluc ./ 9;
...
save(fullfile(matPath, spectrum_mat), "f", "pxx", "v_fluc","u_rms", "U_xt");