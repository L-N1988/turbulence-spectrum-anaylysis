...
pre_C = plot(f_C(f_C <= 30), f_C(f_C <= 30) .* pxx_C(f_C <= 30), 'Color', blue);
hold on
pre_L = plot(f_L, f_L.* pxx_L, 'Color', orange);
pre_CL = plot(concate_f(concate_f <= 30), concate_f(concate_f <= 30) .* concate_pxx(concate_f <= 30), 'Color', green);
