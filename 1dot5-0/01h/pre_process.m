...
% Fs = 24;
u_fluc_tmp = zeros(lt, 1);
pxx = 0; f = 0;
u_fluc = 0; % fluctuation velocity at center
for i = -1:1:1
	for j = -1:1:1
		for k = 1:lt
			u_fluc_tmp(k) = u_pri{k}(center(1) + i, center(2) + j);
		end
		[pxx_, f_] = pwelch(u_fluc_tmp, window_len, [], [], Fs);
        pxx = pxx + pxx_; f = f + f_;
        u_fluc = u_fluc + u_fluc_tmp;
	end
end
pxx = pxx ./ 9; f = f ./ 9;
u_fluc = u_fluc ./ 9;

save(fullfile(matPath, spectrum_mat), "f", "pxx", "u_fluc","u_rms", "U_xt");