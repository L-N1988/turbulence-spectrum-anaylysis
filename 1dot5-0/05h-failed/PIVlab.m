...
save(fullfile(matPath, spectrum_mat), "f", "pxx");

% EMD smooth data
[imf,residual] = emd(pxx);

figure();
plot(f, pxx);
grid on; set(gca, 'XScale', 'log'); set(gca, 'YScale', 'log');
set(xlabel("$f$ (Hz)", "FontSize", 14), 'Interpreter', 'latex'); 
set(ylabel("$S_{uu}(f) (\rm m^2/s)$", "FontSize", 14), 'Interpreter', 'latex');
set(title("PSD", FontSize=14), 'Interpreter', 'latex');
hold on
% xline(0.025:0.025:1, '-.r'); % 1.5:0转速比
% xline(0.05:0.05:1, '-.r'); % 3:0转速比
xline(0.075:0.075:1, '-.r'); % 4.5:0转速比
reconstrct = sum(imf(:, end-3:end), 2) + residual;
h = plot(f, reconstrct, LineWidth=1);

% prefix = strrep(extractAfter(matPath, 'flume\'), '\', '-');
% saveas(h,strcat(prefix, 'PSD'),'epsc')

figure();
plot(f, f.* pxx);
grid on; set(gca, 'XScale', 'log');
set(xlabel("$f$ (Hz)", "FontSize", 14), 'Interpreter', 'latex'); 
set(ylabel("$fS_{uu}(f) (\rm m^2/s^2)$", "FontSize", 14), 'Interpreter', 'latex');
set(title("pre-multiplied PSD", FontSize=14), 'Interpreter', 'latex');
hold on
preh = plot(f, f.*reconstrct, LineWidth=1);
% saveas(preh, strcat(prefix, 'pre-PSD'),'epsc')
