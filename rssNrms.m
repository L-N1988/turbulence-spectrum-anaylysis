...
nexttile
grid on; hold on;
scatter(urms1 ./ u_t(1), z1 / H, 80, "o");
scatter(urms2 ./ u_t(2), z2 / H, 80, "^");
scatter(urms3 ./ u_t(3), z3 / H, 80, "square");
ylim([0, 1]);

set(gca, 'FontSize', 22, 'TickLabelInterpreter', 'latex');
set(ylabel("$z/H$"), 'Interpreter', 'latex');
% set(xlabel("$z/H$"), 'Interpreter', 'latex');
set(xlabel("$\sqrt{\overline{u'^2}}/U_{lid}$"), 'Interpreter', 'latex'); 

% subplot(2, 1, 2)
nexttile
grid on; hold on;
scatter(vrms1 ./ u_t(1), z1 / H, 80, "o");
scatter(vrms2 ./ u_t(2), z2 / H, 80, "^");
scatter(vrms3 ./ u_t(3), z3 / H, 80, "square");
xlim([0.01, 0.03]);
ylim([0, 1]);
...
grid on; hold on;
% p1 = scatter(z1 / H, urms1 ./ u_half(1), 80, "o");
% p2 = scatter(z2 / H, urms2 ./ u_half(2), 80, "^");
% p3 = scatter(z3 / H, urms3 ./ u_half(3), 80, "square");
p1 = scatter(urms1 ./ u_t(1), z1 / H, 80, "o");
p2 = scatter(urms2 ./ u_t(2), z2 / H, 80, "^");
p3 = scatter(urms3 ./ u_t(3), z3 / H, 80, "square");
...
grid on; hold on;
p4 = scatter(vrms1 ./ u_t(1), z1 / H, 80, "o");
p5 = scatter(vrms2 ./ u_t(2), z2 / H, 80, "^");
p6 = scatter(vrms3 ./ u_t(3), z3 / H, 80, "square");
% xlim([0, 0.1]);
ylim([0, 1]);
...
grid on; hold on;
p7 = scatter(rss1 ./ u_t(1)^2, z1 / H, 80, "o");
p8 = scatter(rss2 ./ u_t(2)^2, z2 / H, 80, "^");
p9 = scatter(rss3 ./ u_t(3)^2, z3 / H, 80, "square");
% xlim([0, 0.1]);
ylim([0, 1]);

set(gca, 'FontSize', 22, 'TickLabelInterpreter', 'latex');
set(xlabel("$-\overline{u'w'}/U^2_{lid}$"), 'Interpreter', 'latex'); 
set(ylabel("$z/H$"), 'Interpreter', 'latex');
hLg = legend({'1.5:0', '3.0:0', '4.5:0'}, 'Location', 'best', "FontSize", 22, 'Interpreter', 'latex');
title(hLg, '$\omega_t:\omega_b$', 'Interpreter','latex');
...
% figure;
% hold on
% cellfun(@scatter, urms, Z);
% title("urms")
% hold off

% figure;
% hold on
% cellfun(@scatter, vrms, Z);
% title("vrms")
% hold off

% figure;
% hold on
% cellfun(@scatter, rss, Z);
% title("rss")
% hold off